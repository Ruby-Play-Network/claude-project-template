...
The app deploys to Lyra, the deploy manager for `vegaverse.net`
(`https://www.vegaverse.net/_lyra`; the OpenAPI spec at `/api/openapi.json` is
the authority, the docs and skill files are summaries).

**It deploys as a single Lyra _service_, not a site plus a service.** Lyra serves
a static "site" from `www.vegaverse.net/<name>/` and a git-backed "service" from
`<name>.vegaverse.net/`. Because the front end needs the API, the API process
serves the built front end too, from the same origin: `apps/api/src/index.ts`
mounts `apps/web/dist` and falls back to the SPA shell for non-API GETs. One
origin means no CORS, no Vite `base` path, and none of the manual nginx
`try_files` line a separately-hosted SPA would need.

Constraints the platform imposes, all currently met:

- **Listen on `process.env.PORT`.** Lyra allocates a loopback port; a hard-coded
  port fails the health check. `platform/config/` reads `PORT`, and Lyra will not
  let `env` override it.
- **Expose a health path.** `GET /healthz` (`features/health/`) answers non-5xx
  once the app is up. A failed health check rolls the release back automatically.
- **The build must work from a clean clone.** `apps/web/.env.production` is
  committed for exactly this reason: `.env` is gitignored, so without it
  `VITE_API_BASE_URL` is `undefined` at build time and the browser fetches
  `undefined/app-info`. TypeScript does not catch this — `env.d.ts` types it as
  `string`.

Service definition (`POST /api/services`):

```json
{
  "name": "acrux",
  "title": "Acrux",
  "repo": "git@github.com:Ruby-Play-Network/Acrux.git",
  "branch": "main",
  "buildCmd": "corepack enable && pnpm install --frozen-lockfile && pnpm build",
  "startCmd": "pnpm --filter api start",
  "healthPath": "/healthz",
  "database": true,
  "env": { "NODE_ENV": "production", "APP_NAME": "Acrux" }
}
```

`database: true` is not optional here: `platform/config/` requires
`DATABASE_URL`, so the service will not boot without the one Lyra injects.

**`releaseCmd` is deliberately unset until the first migration exists.** It runs
before the release is flipped, and a failure fails the deploy, so there is
nothing to gain from running a migrator against an empty `migrations/` folder.
When the first table lands, set it to `pnpm --filter api db:migrate`. Migrations
belong there and never in `startCmd`, which re-runs on every restart and on every
crash-loop iteration.

Creating a service does not deploy it. The 201 returns a `deployKey` to add to
the GitHub repo as a read-only deploy key (the repo is private, so the clone
fails until that is done) and a `webhookUrl`/`webhookSecret` pair shown **once**.
Then `POST /api/services/acrux/deploy` and poll `GET /api/builds/<id>`.

Lyra accepts the shared `LYRA_ADMIN_TOKEN` (break-glass), an Arcturus session
token (`st_…`), or an Arcturus user API key (`uk_…`), in `Authorization: Bearer`
and never in `?token=`. An `mk_…` key is an Arcturus _operator management key_ —
a different credential, not a Lyra one.
